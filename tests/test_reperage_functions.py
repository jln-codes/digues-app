...
class CabbalrReperageIntegrationTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        try:
            import psycopg
            ...
            config = PostgreSQLConfig.from_env()
            cls.connection = psycopg.connect(
                **config.connect_kwargs(autocommit=True), row_factory=dict_row
            )
            cls.cursor = cls.connection.cursor()
            cls.cursor.execute(
                """
                SELECT EXISTS (
                    SELECT 1 FROM public.troncons
                    WHERE id = 'f6e9b93f-7eb7-7cd5-0679-44d7db80fdb2'
                ) AND to_regprocedure(
                    'public.xy_vers_reperage(uuid,uuid,geometry)'
            ...
            if not cls.cursor.fetchone()["available"]:
                raise unittest.SkipTest("Corpus cabbalr non migré dans PostgreSQL.")
        except unittest.SkipTest:
            raise
        except Exception as exc:
            raise unittest.SkipTest(f"PostGIS local indisponible : {exc}")
    ...
    def test_coord_disorder_matches_audited_projection(self):
        self.cursor.execute(
            """
            SELECT statut, statut_pr, distance_axe_m, distance_borne_m,
                   abscisse_m, pr, borne_id,
                   ST_AsText(point_projete) AS point_projete
            FROM public.xy_vers_reperage(
                'f6e9b93f-7eb7-7cd5-0679-44d7db80fdb2',
                'f6e9b93f-7eb7-7cd5-0679-44d7db81186b',
                ST_SetSRID(
                    ST_Point(1673689.08982578, 9259313.60364733), 3950
                )
            )
            """
        )
        ...
        self.assertEqual((result["statut"], result["statut_pr"]), ("OK", "OK"))
        self.assertAlmostEqual(result["distance_axe_m"], 1.876369488, places=8)
        self.assertAlmostEqual(result["distance_borne_m"], 43.628211895, places=8)
        self.assertAlmostEqual(float(result["pr"]), 43.628212555, places=8)
        self.assertEqual(
            str(result["borne_id"]), "f6e9b93f-7eb7-7cd5-0679-44d7db8126ef"
        )

    def test_linear_disorder_matches_audited_signed_offsets(self):
        for offset, expected_pr, expected_point in (
            (-10.0, 103.072250365, "POINT(1654759.758881805 9268314.775767505)"),
            (-5.0, 108.072250183, "POINT(1654756.311571662 9268318.397376647)"),
        ):
            with self.subTest(offset=offset):
                self.cursor.execute(
                    """
                    SELECT statut, statut_pr, abscisse_borne_m, pr,
                           ST_AsText(point_xy) AS point_xy
                    FROM public.borne_offset_vers_xy(
                        '45968145-14e1-e417-02e0-675e6f1b8fd4',
                        '45968145-14e1-e417-02e0-675e6f1b9e88',
                        'f6e9b93f-7eb7-7cd5-0679-44d7db81a04a', %s
                    )
                    """,
                    (offset,),
                )
                result = self.cursor.fetchone()
                self.assertEqual(
                    (result["statut"], result["statut_pr"]), ("OK", "OK")
                )
                self.assertAlmostEqual(
                    result["abscisse_borne_m"], 113.072254129, places=8
                )
                self.assertAlmostEqual(float(result["pr"]), expected_pr, places=8)
                self.assertEqual(result["point_xy"], expected_point)

    def test_prestation_is_a_read_only_control_not_a_migrated_table(self):
        expected = (
            ("45968145-14e1-e417-02e0-675e6f1bbca5", 0.0),
            ("f6e9b93f-7eb7-7cd5-0679-44d7db81a04a", 113.07225),
        )
        for borne_id, expected_pr in expected:
            with self.subTest(borne_id=borne_id):
                self.cursor.execute(
                    """
                    SELECT statut, statut_pr, pr
                    FROM public.borne_offset_vers_xy(
                        '45968145-14e1-e417-02e0-675e6f1b8fd4',
                        '45968145-14e1-e417-02e0-675e6f1b9e88', %s, 0
                    )
                    """,
                    (borne_id,),
                )
